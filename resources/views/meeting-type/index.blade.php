...
@section('breadcrumbs')
    {{ Breadcrumbs::render('corsec.meeting-type') }}
@endsection
...
@section('content')
    ...
    <div class="grid">
        <div class="min-w-full card card-grid" data-datatable="false" data-datatable-page-size="10"
            data-datatable-state-save="false" id="meeting-type-table"
            data-api-url="{{ route('meeting-type.datatables') }}" data-base-url="{{ route('meeting-type.index') }}">
                ...
                <div class="flex flex-wrap gap-2 lg:gap-5">
                    <div class="flex">
                        <label class="input input-sm"> <i class="ki-filled ki-magnifier"> </i>
                            <input placeholder="Search Meeting Type" id="search" type="text" value="">
                        </label>
                    </div>
                    <div class="flex flex-wrap gap-2.5">
                        <div class="h-[24px] border border-r-gray-200"></div>
                        @if ($permissionFlags['can_export'] ?? false)
                            <a id="export-btn" class="btn btn-sm btn-light" href="{{ route('meeting-type.export') }}">
                                Export to Excel
                            </a>
                        @endif
                        @if ($permissionFlags['can_create'] ?? false)
                            <a class="btn btn-sm btn-primary" href="{{ route('meeting-type.create') }}"> Tambah Meeting Type
                            </a>
                        @endif
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="min-w-[100px]" data-datatable-column="code">
                                    <span class="sort"> <span class="sort-label"> Kode Meeting Type </span>
                                        <span class="sort-icon"> </span> </span>
                                </th>
                                <th class="min-w-[250px]" data-datatable-column="name">
                                    <span class="sort"> <span class="sort-label"> Nama Meeting Type </span>
                                        <span class="sort-icon"> </span> </span>
                                </th>
...
@push('scripts')
    <script type="text/javascript">
        function deleteData(rowKey) {
            const element = document.querySelector('#meeting-type-table');
        ...
        function deleteSelectedRows() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax('{{ route('meeting-type.deleteMultiple') }}', {
                        type: 'POST',
                        data: {
                            ids: ids
                        }
    ...
    <script type="module">
        ...
        const dataTableOptions = {
            ...
            columns: {
                ...
                code: {
                    title: 'Kode Meeting Type',
                },
                name: {
                    title: 'Nama Meeting Type',
                },
                description: {
                    title: 'Deskripsi',
                },
