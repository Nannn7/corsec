...
@section('breadcrumbs')
    {{ Breadcrumbs::render(isset($sender) ? 'sender.edit' : 'sender.create') }}
@endsection
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    ...
                    {{ isset($sender) ? 'Edit Sender' : 'Tambah Sender' }}
                </h3>
                <a href="{{ route('sender.index') }}" class="btn btn-sm btn-info">
            ...
            <div class="card-body">
                <form method="POST" action="{{ isset($sender) ? route('sender.update', $sender->id) : route('sender.store') }}">
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div class="flex flex-col">
                            <label class="form-label">Nama Sender <span class="text-danger">*</span></label>
                            <input class="input @error('name') border-danger bg-danger-light @enderror" type="text"
                                name="name" value="{{ old('name', $sender->name ?? '') }}" maxlength="150" required>
                            @error('name')
                                <em class="mt-1 text-sm alert text-danger">{{ $message }}</em>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col md:col-span-2">
                            ...
                            <textarea class="textarea @error('description') border-danger bg-danger-light @enderror" name="description"
                                rows="4" placeholder="Keterangan tambahan...">{{ old('description', $sender->description ?? '') }}</textarea>
                            @error('description')
                                <em class="mt-1 text-sm alert text-danger">{{ $message }}</em>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col">
                            ...
                            @php
                                $statusValue = old('status', isset($sender) ? (int) $sender->status : 1);
                            @endphp
                    ...
                    <div class="flex justify-end mt-7 gap-2">
                        <a href="{{ route('sender.index') }}" class="btn btn-light">
                            Cancel
                        </a>
                        @can(isset($sender) ? 'sender.update' : 'sender.create')
